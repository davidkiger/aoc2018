...
while True:
    targets = []
    groups = sorted(groups, key=lambda x: (x['units'] * x['damage'], x['initiative']), reverse=True)
    chosen = []
    for a, attacker in enumerate(groups):
        ...
        target_ep = 0
        target_init = 0
        power = attacker['units'] * attacker['damage']
        for d, defender in enumerate(groups):
            ...
            if potential_damage > projected_damage:
                ...
                target_ep = defender['units'] * defender['damage']
                target_init = defender['initiative']
                projected_damage = potential_damage
            elif potential_damage == projected_damage:
                if defender['units'] * defender['damage'] > target_ep or (defender['units'] * defender['damage'] == target_ep and defender['initiative'] > target_init):
                    target_id = defender['group_id']
                    target_ep = defender['units'] * defender['damage']
                    target_init = defender['initiative']
    ...
    for target in targets:
        ...
        damage = attacker['units'] * attacker['damage']
